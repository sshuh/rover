...
// ----------------- Tcp Client -------------------
	...
	int RecvStoreLine1( unsigned char *ucBuf, int nMaxRead, int nReadTimeout, int iPrintConsole=0 );
...
int HS2_TcpipComm::RecvStoreLine1( unsigned char *ucBuf, int nMaxRead, int nReadTimeout, int iPrintConsole )
{
	int i;
	for( i=0; i<(nMaxRead-1); ++i)
	{
		unsigned char ucCharRecv = 0;
		int nBytesRead;
		nBytesRead = recv( m_hClntSock, (char*)(&ucCharRecv), 1, 0 );
		if( nBytesRead <= 0 )
		{
			if(i == 0)
			{
